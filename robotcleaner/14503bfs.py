...
def bfs():
    while q:
        ...
        if method == 2: # 현 좌표로부터 네 방향에서 모두 청소가 되어 있을 때 
            nrow, ncol, narrow = moving_2(row,col, arrow)
            if matrix[nrow][ncol] == 1: # 만약에 벽이라면 bfs 정지. 즉 로봇 정지
                return
            else:
                q.append([nrow,ncol,arrow]) # 만약 벽이 아니라면 q 로 이동
        if method == 3: # 현 좌표로부터 네 방향에서 청소되지 않은 것이 있을 때
            narrow = rotation(arrow) # arrow 의 방향 반시계 90도로 회전.
            nrow, ncol = moving_3(row,col,narrow)
            if matrix[nrow][ncol] == 0:
                q.append([nrow,ncol,narrow])
            else:
                q.append([row,col,narrow])
